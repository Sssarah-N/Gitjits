...
saved_park_model = auth_ns.model('SavedPark', {
    'park_code': fields.String(required=True, description='Park code')
})
...
class Register(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            return {'Error': f'Registration failed: {str(err)}'}, 500
...
class SavedPark(Resource):
    ...
    @auth_ns.doc(params={'park_code': 'Park code to remove'},
                 description='Remove park from saved list',
                 security='Bearer')
    @token_required
    def delete(self, park_code, current_user):
        """Remove a park from saved list."""
        try:
            removed = uqry.remove_saved_park(
                current_user['username'], park_code
            )

            if removed:
                return {'message': f'Park {park_code} removed'}, 200
            else:
                return {'Error': f'Park {park_code} not in saved'}, 404
        except ValueError as err:
            return {'Error': str(err)}, 400
        except Exception as err:
            return {'Error': str(err)}, 500
...
class CreateAdminUser(Resource):
    ...
    def post(self, current_user):
        ...
        try:
            ...
            return {'Error': f'Admin creation failed: {str(err)}'}, 500
